...
(function () {
  ...
  var fillScale = d3.scaleOrdinal().range(['#FC8D59', '#91CF60', '#91BFDB']);
  ...
  var svg = d3.select('#chart2').attr('width', width).attr('height', height);

  // Get the rectangles inside of it
  svg.selectAll('rect').data(countries).attr('width', function (d) {
    return widthScale(d.gdp);
  }).attr('height', 50).attr('fill', function (d) {
...
//# sourceMappingURL=/02-chart.f40eb0e0.map